...
|---|---|---|---|
| Rust | fn, struct, enum, trait, impl, type alias, const/static, macro, every `let` binding at any depth (simple and tuple patterns) | Cross-crate via a discovered workspace `crate_map`, `crate::`/`super::`/`self::` paths, re-export chasing (depth-capped) | Struct/enum destructuring `let` patterns aren't walked for names (simple and tuple patterns are); otherwise no known gaps |
| Python | Functions (incl. decorated, async), classes, every assignment target at any depth (module/local/`self.attr`, tuple unpacking) | Relative (`from . import x`) and absolute (`from pkg.mod import x`) imports, resolved against real files on disk | None currently known — see `tests/test_import_resolution.py` for exact coverage |
| TypeScript / JS | Functions, classes, interfaces, enums, type aliases, every `const`/`let`/`var` at any depth (function-valued ones recorded as functions) | Relative imports (`./foo`, `../bar`) | `tsconfig.json` path-alias imports (`@app/utils`) are **not** resolved and are classified external, same as a real npm package |
| Go | Functions, methods, structs, interfaces, every `var`/`const`/`:=` declaration at any depth (blank identifier `_` excluded) | Same-module imports via `go.mod`'s declared module path; resolves to a representative file in the target package directory | Struct/type signature text omits the leading `type` keyword (cosmetic — name/kind/resolution are unaffected). No `go.mod` means nothing resolves as internal at all (there's no reliable way to tell a same-module import from a third-party one without it) |
| Java | Classes, methods (incl. generics, annotations), every field and local variable declaration | Package-path-to-file resolution against project root, `src/`, and `src/main/java/`; known JDK prefixes short-circuit to external without a filesystem check | None currently known |
| C | Functions, every global/local variable declaration (multi-declarator statements, `static`/`extern`, arrays, pointers) | `#include "local.h"` (quoted) resolves against source-relative, project-root, and common include dirs; `#include <system.h>` (angle-bracket) always stays external, by C's own syntax | `#define` macros are **not** extracted as signatures at all |
| C++ | Classes (incl. templates), functions, every global/local variable declaration (same coverage as C) | Same quote/angle-bracket handling as C | Same macro gap as C. Templates are captured as plain classes/functions — no separate generic-parameter extraction |
| Ruby | Methods, singleton methods, classes, modules, every assignment target (locals, globals, `@ivar`/`@@cvar`, constants, multi-assignment) | `require_relative` (relative), plain `require` (project root / `lib/` / `app/`), `load`/`autoload` (same lookup as `require`) | `include`/`extend`/`prepend` reference an already-loaded constant, not a file — never resolve, which is correct, not a gap |

Variable/field extraction, like signatures, is Tree-sitter AST dispatch, never regex or a hand-rolled parser — see `tests/test_extractors.py` for exact per-language coverage, including the specific duplicate- and phantom-extraction regressions each grammar's `declaration`-style overloaded node shapes required guarding against (e.g. a `struct Foo instance;` referencing an existing type must never be reported as a second definition of `Foo`).

An AI answering "how many times is X referenced" or "are these two names suspiciously similar" doesn't need a separate tool for either: `audit_search` already reports an exact per-file occurrence count against the complete indexed content (every file, not just these 8 languages — see above), and every extracted name is visible through `audit_read_file`/`audit_search` for the AI's own judgment — SlugAudit surfaces the evidence, it doesn't run the analysis itself (see "Project contract").
