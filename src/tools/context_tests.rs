...
fn a_verified_connection_succeeds_when_nothing_changed_since_sync() {
    ...
    let synced = ensure_synced(&project.path().to_string_lossy(), &cache()).expect("sync");
...
fn a_stale_synced_handle_fails_loudly_instead_of_returning_mismatched_data() {
    let project = activated_project("lib.rs", b"pub fn a() {}\n");
    let path = project.path().to_string_lossy().into_owned();
    let stale = ensure_synced(&path, &cache()).expect("first sync");
    ...
    .expect("modify file");
    let fresh = ensure_synced(&path, &cache()).expect("second sync");
    assert_ne!(
...
fn a_database_copied_from_a_different_project_root_fails_closed() {
    let project = activated_project("lib.rs", b"pub fn a() {}\n");
    let path = project.path().to_string_lossy().into_owned();
    ensure_synced(&path, &cache()).expect("first sync establishes the project row");
    ...
    let result = ensure_synced(&path, &cache());
    assert!(
        result.is_err(),
...
/// When the recency cache reports a recent sync, `ensure_synced` must skip
/// the full publish (discovery + sample + diff) and return the current
/// revision id directly. Proved by syncing, modifying a file on disk, then
/// syncing again with the same cache within the window: if the publish ran,
/// the revision would move; if it was skipped, the revision stays put.
#[test]
fn a_recent_cache_hit_skips_the_full_publish_even_when_files_changed() {
    let project = activated_project("lib.rs", b"pub fn a() {}\n");
    let path = project.path().to_string_lossy().into_owned();
    let cache = cache();

    let first = ensure_synced(&path, &cache).expect("first sync");

    // Change the file on disk. A full publish would detect this and
    // advance the revision; a cache hit must not.
    fs::write(
        project.path().join("lib.rs"),
        b"pub fn a() { changed(); }\n",
    )
    ...
    let second = ensure_synced(&path, &cache).expect("second sync within recency window");
    assert_eq!(
        first.revision_id, second.revision_id,
        "a recency-cache hit must skip the publish entirely, leaving the revision unchanged \
         even though the file was modified on disk"
    );

    // The returned revision is still valid for a read: the DB wasn't
    // republished, so the revision check passes.
    let count: i64 = with_verified_read(&second, |tx| {
        tx.query_row("SELECT count(*) FROM files", [], |row| row.get(0))
            .map_err(|error| ErrorData::internal_error(error.to_string(), None))
    })
    .expect("cached revision still opens fine");
    assert_eq!(count, 1_i64);
}

/// A cache miss (fresh cache, or cache expired) must run the full publish.
/// Two syncs with independent caches against an unchanged project both
/// succeed and return the same revision id — the second one did the work
/// rather than trusting the first.
#[test]
fn a_cache_miss_runs_the_full_publish_and_returns_the_current_revision() {
    let project = activated_project("lib.rs", b"pub fn a() {}\n");
    let path = project.path().to_string_lossy().into_owned();

    let first = ensure_synced(&path, &cache()).expect("first sync");
    // Independent cache — no recency credit from the first sync.
    let second = ensure_synced(&path, &cache()).expect("second sync with fresh cache");
